//
//  StorefrontAPIQueriesTests.swift
...
final class StorefrontAPIQueriesTests: XCTestCase {
    ...
    func testCartQuerySuccessWithCompleteJSON() async throws {
        // Given - Complete valid JSON response
        let json = """
        {
            "data": {
                "cart": {
    ...
    func testCartQueryWithPartialDataAndErrors() async throws {
        // Given - Response with both data and errors
        let json = """
        {
            "data": {
                "cart": {
        ...
        // When/Then - Should throw because GraphQLClient throws on any errors
        do {
            _ = try await storefrontAPI.cart(by: GraphQLScalars.ID("gid://shopify/Cart/123"))
    ...
    // MARK: - Products Query Tests with Real JSON

    func testProductsQuerySuccessWithCompleteJSON() async throws {
        // Given - Complete products response
        let json = """
        {
            "data": {
                "products": {
        ...
        // When
        let products = try await storefrontAPI.products(first: 2)

        // Then
        XCTAssertEqual(products.count, 2)
        XCTAssertEqual(products[0].title, "T-Shirt")
        XCTAssertEqual(products[0].vendor, "Clothing Co")
        XCTAssertNotNil(products[0].featuredImage)
        XCTAssertNotNil(products[0].variants)
        XCTAssertEqual(products[0].variants?.nodes.count, 1)
        XCTAssertEqual(products[1].title, "Hoodie")
        XCTAssertNil(products[1].featuredImage)
    }

    func testProductsQueryEmptyResult() async throws {
        // Given - Empty products list
        let json = """
        {
            "data": {
                "products": {
        ...
        // When
        let products = try await storefrontAPI.products(first: 10)
    ...
    func testProductsQueryWithNullableFields() async throws {
        // Given - Products with nullable fields
        let json = """
        {
            "data": {
                "products": {
        ...
        // When
        let products = try await storefrontAPI.products(first: 1)

        // Then
        XCTAssertEqual(products.count, 1)
        XCTAssertNil(products[0].id)
        XCTAssertEqual(products[0].title, "Product Without ID")
        XCTAssertNil(products[0].vendor)
    }

    func testProductsQueryWithInvalidProductStructure() async {
        // Given - Invalid product structure
        let json = """
        {
            "data": {
                "products": {
        ...
        // When/Then
        do {
            _ = try await storefrontAPI.products(first: 1)
            XCTFail("Should throw decoding error")
        } catch {
            // Expected error
            XCTAssertTrue(error is DecodingError || (error as? GraphQLError) != nil)
    ...
    func testProductsQueryWithUnicodeAndSpecialCharacters() async throws {
        // Given - Products with unicode and special characters
        let json = """
        {
            "data": {
                "products": {
                    "nodes": [
                        {
                            "id": "gid://shopify/Product/1",
                            "title": "T-Shirt with 🎉 emoji",
                            "vendor": "Vendor™ with ® symbols",
                            "featuredImage": null,
                            "variants": null
                        },
                        {
        ...
        // Then
        XCTAssertEqual(products.count, 2)
        XCTAssertEqual(products[0].title, "T-Shirt with 🎉 emoji")
        XCTAssertEqual(products[0].vendor, "Vendor™ with ® symbols")
        XCTAssertEqual(products[1].title, "Product with\nnewline and\ttab")
        XCTAssertEqual(products[1].vendor, "Vendor with \"quotes\"")
    }
    ...
    func testQueryWithTimeout() async {
        ...
        // When/Then
        do {
            _ = try await storefrontAPI.products(first: 10)
    ...
    func testCartQuerySendsCorrectVariables() async throws {
        ...
        // Then - Check the request was formed correctly
    ...
    func testProductsQuerySendsCorrectVariables() async throws {
        // Given
        let json = """
        {"data": {"products": {"nodes": []}}}
        """
        ...
        // When
        _ = try await storefrontAPI.products(first: 25)
        ...
        do {
            ...
            let variables = jsonBody?["variables"] as? [String: Any]
            XCTAssertEqual(variables?["first"] as? Int, 25)
        } catch {
            XCTFail("Failed to parse request body: \(error)")
        }
    }
